    ...
    def data(self, index, role=Qt.DisplayRole):
        if role == Qt.DisplayRole:
            if not self.list.indexWidget(index):
                #aaa = loadUi('ms_item.ui')
                self.list.setIndexWidget(index, self.items[index.row()])
                return QVariant()   #self.items[index.row()]   #'x'
#                return self.items[index.row()][index.column()]
    ...
    def removeRows(self, rowIndexes):
        ...
        for row in sorted(rowIndexes, reverse=True):
            ...
            self.endRemoveRows()
        '''
        self.beginRemoveRows(QModelIndex(), rowIndexes[0], rowIndexes[0])
        del self.items[rowIndexes[0]]
        self.endRemoveRows()

    def addRow(self, id, title, res1, res2, res3):
        self.beginInsertRows(QModelIndex(), len(self.items), 1)
        aaa = loadUi('ms_item.ui')
        aaa.textBrowser.setPlainText(str(uuid.uuid4()))
